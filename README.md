...
- 사이트 호환 모드: 넷플릭스처럼 고배속에서 싱크가 밀릴 수 있는 플레이어는 안정적인 최대 배속을 적용합니다.
- 현재 사이트 제외: 문제가 나는 사이트에서는 팝업에서 해당 사이트만 확장 동작을 끌 수 있습니다.
...
넷플릭스에서 영상이나 음성이 밀리면 `사이트 호환 모드`를 켠 상태로 사용합니다. 호환 모드에서는 넷플릭스 배속을 최대 1.5배속으로 제한합니다. 그래도 밀림이 있으면 팝업에서 `현재 사이트에서 끄기`를 눌러 넷플릭스에서만 확장을 제외합니다.
